...
@custom_jvp
def cauchy_activation(x, lambda1=2.287478, lambda2=-0.000559, d=0.909817):
...
def cauchy_activation_jvp(primals, tangents):
    ...
    d_safe = jnp.sqrt(
        d**2 + 1e-6
    )  # se |d|  << 1e-3, d_safe ≃ 1e-3. Caso contrário d_safe ≃ d
    ...
    tangent_out = (
        dy_dx * x_dot
        + dy_dlambda1 * lambda1_dot
        + dy_dlambda2 * lambda2_dot
        + dy_dd * d_dot
    )
...
class CauchyActivationModule(nnx.Module):
    def __init__(
        self, init_lambda1: float = 1.0, init_lambda2: float = 0.0, init_d: float = 1.0
    ):
