...
class Actuators:
    ...
    @classmethod
    def _from_joint_ids(
        cls, mj_model, joint_ids: jax.Array
    ) -> tuple[MaybeM[list[int]], MaybeM[list[str]]]:
    ...
    def on_range_by_id(self, actuator_values: jax.Array):
        ...
        (_, on_range), _ = jax.lax.scan(
            on_range_single, (0, True), length=self.ids.shape[0]
        )
        return on_range
        ...
        for id in ids:
            qposadr.append(
                jnp.arange(
                    ...
                    mj_model.jnt_qposadr[id]
                    + Joints.joint_infoby_typecode(mj_model.jnt_type[id])["qpos_width"],
                )
            )
            qveladr.append(
                jnp.arange(
                    ...
                    mj_model.jnt_dofadr[id]
                    + Joints.joint_infoby_typecode(mj_model.jnt_type[id])["dof_width"],
                )
            )
    ...
    def joint_infoby_typecode(cls, tp: int):
        _map = {
            0: {
                ...
                "dof_width": 6,
                "qpos_width": 7,
            },  # 3 valores de posição + 4 para rotação (quaternion)
            1: {
                "type": "ball",
                "dof_width": 3,
                "qpos_width": 4,
            },  # 4 valores apenas rotação (quaternion)
            2: {
                "type": "slide",
                "dof_width": 1,
                "qpos_width": 1,
            },  # 1 grau de liberdade. 1 valor
            3: {
                "type": "hinge",
                "dof_width": 1,
                "qpos_width": 1,
...
class SafeActionSpace:
    ...
    @classmethod
    def init(cls, length, actuators: Actuators) -> Self:
        return cls(jnp.zeros((length, actuators.number_of)), actuators)
...
def sample_config_coordinates(rsd: RobotSharedData, rng, config_name: str):
    """
...
def sample_config_velocities(rsd: RobotSharedData, rng, config_name: str):
    """
...
def get_goal(range_config: RangeConfig, progress, rng):
    ...
    rng, position_velocities = range_config.position_velocities.sample_normal(
        rng, progress
    )
        ...
        env.bind(
            lambda pdata: StateMonad(
                lambda state: (
                    ...
                    {
                        **pdata,
                        "tool_position": rsd.sensor_data(
                            state["mjx_data"], "tool_position"
                        ),
                    },
                )
            )
                    ...
                    state,
                    {
                        **pdata,
                        "position_error": position_error(
                            state["goal"]["goal_position_coordinates"],
                            pdata["tool_position"],
                        ),
                    },
                )
